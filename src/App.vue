<template>
	<NcContent app-name="jeslinks" class="app-Werkstaat">
		<NcAppNavigation>
			<div>
				<div v-if="isAdmin" class="adminvakje">
					<h2 class="titelAN">
						Enkel voor admin
					</h2>
					<ul>
						...
						<ActionInput icon="icon-search" @change="perPersoon($event.target.value)" />
						<div style="width: 100%; display: block; margin-bottom: 10px;" />
						<NcAppNavigationItem v-for="pnote in AanvraagPAr" :key="pnote.id"
							:title="pnote.voornaam + ' ' + pnote.achternaam"
							:name="pnote.voornaam + ' ' + pnote.achternaam"
							@click="toonWLinksVan(pnote.uid, pnote.emailinvoer)" />
						<NcAppNavigationNew v-if="isAdmin" text="Afgeschermde werklink aanmaken"
							title="Afgeschermde werklink aanmaken" button-class="icon-add" @click="newWNote" />
					</ul>
				</div>
				<h2 class="titelAN">
					JES links
				...
				<ul>
					<NcAppNavigationItem title="Afgeschermde werklinks" name="Afgesschermde werklinks"
						class="werklinknavM" @click="toonWLinks()" />
					<NcAppNavigationItem v-for="note in AanvraagAr" :key="note.id" :title="note.categorie"
						:name="note.categorie" :class="{ active: currentNoteId === note.id }"
						@click="toonLinks(note.categorie, JESL, 'jeslinks')" />
				</ul>
				<NcAppNavigationNew v-if="(isAdmin && AanvraagAr.length === 0)" :disabled="false"
					text="Een JES-link toevoegen" button-id="new-Aanvragen-button" button-class="icon-add"
					@click="newNote" />
				<h2 class="titelAN">
					Jouw favorieten
				</h2>
				<ul>
					<NcAppNavigationItem v-for="notep in AanvraagPrivAr" :key="notep.id" :title="notep.categorie"
						:name="notep.categorie" :class="{ active: currentPNoteId === notep.id }"
						@click="toonLinks(notep.categorie, notep.wie, 'persoonlijk')" />
				</ul>
				<NcAppNavigationNew v-if="AanvraagPrivAr.length === 0" text="Een link toevoegen" :disabled="false"
					button-id="new-Aanvragen-button" button-class="icon-add" @click="newPNote" />
			</div>
			<template #footer>
				<NcAppNavigationSettings title="meer">
					<ul>
						<NcAppNavigationItem title="eerst opvragen..." @click="toekenbaar">
							<template slot="actions">
								<ActionButton icon="icon-info" @click="toonConfigWLinks">
									dan tonen...
								</ActionButton>
							</template>
						</NcAppNavigationItem>
		...
		<NcAppContent>
			...
			<div v-if="(currentPNote && lengteAr >= 1 && priveAdmin && Plinks)" class="vraagcontainer">
				<div class="in-takenvak">
					<h3>Een link aanmaken:</h3>
					<input ref="wie" v-model.trim="currentPNote.wie" type="hidden" placeholder="userId"
						:disabled="updating">
					<input ref="email" v-model="currentPNote.email" type="hidden" placeholder="email"
						:disabled="updating">
					<input ref="url" v-model="currentPNote.url" type="text" placeholder="De link/url"
						:disabled="updating">
					<input ref="naam" v-model="currentPNote.naam" type="text" placeholder="Een naam voor de link"
						:disabled="updating">
					<input ref="pasw" v-model="currentPNote.pasw" type="hidden" placeholder="een eventueel paswoord"
						:disabled="updating">
					<input ref="categorie" v-model="currentPNote.categorie" type="text"
						placeholder="Een categorie voor de link" :disabled="updating">
					<input ref="groep" v-model="currentPNote.groep" type="hidden"
						placeholder="Welke JES groep krijgt automatisch deze link" :disabled="updating">
					<input type="button" class="primary" value="Opslaan" :disabled="updating || !savePPossible"
						@click="savePNote">
					<input type="button" class="secondary" value="Annuleren" :disabled="updating || !savePPossible"
						@click="cancelPNote">
				</div>
			</div>
			<div v-if="(currentNote && lengteAr >= 1 && jesAdmin && Jlinks)" class="vraagcontainer">
				<div class="in-takenvak">
					<h3>Een JES-link aanmaken:</h3>
					<input v-model.trim="currentNote.wie" type="hidden" placeholder="userId" :disabled="updating">
					<input v-model="currentNote.email" type="hidden" placeholder="email" :disabled="updating">
					<input ref="urlJ" v-model="currentNote.url" type="text" placeholder="De link/url"
						:disabled="updating">
					<input ref="naam" v-model="currentNote.naam" type="text" placeholder="Een naam voor de link"
						:disabled="updating">
					<input v-model="currentNote.pasw" type="hidden" placeholder="een eventueel paswoord"
						:disabled="updating">
					<input v-model="currentNote.categorie" type="text" placeholder="Een categorie voor de link"
						:disabled="updating">
					<input v-model="currentNote.groep" type="hidden"
						placeholder="Welke JES groep krijgt automatisch deze link" value="iedereen"
						:disabled="updating">
					<input type="button" class="primary" value="Opslaan" :disabled="updating || !savePossible"
						@click="saveNote">
					<input type="button" class="secondary" value="Annuleren" :disabled="updating || !savePPossible"
						@click="cancelJNote">
				</div>
			</div>
			<div v-if="(currentWNote && lengteAr >= 1 && werkAdmin && TKAlinks)" class="vraagcontainer">
				<div class="in-takenvak">
					<h3>Een afgeschermde link aanmaken:</h3>
					<input v-model.trim="currentWNote.wie" type="hidden" placeholder="userId" :disabled="updating">
					<input v-model="currentWNote.email" type="hidden" placeholder="email" :disabled="updating">
					<input ref="urlW" v-model="currentWNote.url" type="text" placeholder="De link/url"
						:disabled="updating">
					<input ref="naam" v-model="currentWNote.naam" type="text" placeholder="Een naam voor de link"
						:disabled="updating">
					<input v-model="currentWNote.pasw" type="password" placeholder="een eventueel paswoord"
						:disabled="updating">
					<span>
						Paswoord ingeven of standaard paswoord gebruiken
					</span>
					<br>
					<input v-model="currentWNote.categorie" type="hidden" :disabled="updating">
					<input v-model="currentWNote.groep" type="hidden" :disabled="updating">
					<input type="button" class="primary" value="Opslaan" :disabled="updating || !saveWPossible"
						@click="saveWNote">
					<input type="button" class="secondary" value="Annuleren" :disabled="updating || !saveWPossible"
						@click="cancelWNote">
				</div>
			</div>
			<div v-if="Jlinks" class="vraagcontainer">
				<h3 class="h3inline">
					JES links: {{ gekozenJCategorie }}
				</h3>
				<input v-if="isAdmin" :disabled="false" type="button" text="Een JES-link toevoegen"
					button-id="new-Aanvragen-button" class="icon-add toevoegknopje" @click="newNote">
				<div v-for="(klik, k) in KlikbaarAr" :key="k" class="linklijn">
					<a :href="klik.url" target="_blank">{{ klik.naam }}</a>
					<input type="button" class="icon-clippy copknopje" title="kopieer link naar klembord"
						@click="klembord(klik.url, $event)">
					<input v-if="isAdmin" type="button" class="icon-delete delknopje"
						@click="linkWeg(klik.id, 'jeslinks')">
				</div>
			</div>
			<div v-if="TKlinks" class="vraagcontainer">
				<h3 class="h3inline">
				...
				<div v-for="(klik, k) in KlikbaarAr" :key="k" class="linklijn">
					...
					<input v-if="klik.opslagknop" class="success toekenknop" type="button" style="display: inline"
						value="Werklink schrappen" @click="linkWeg(klik.teschrappen, 'TK')">
					<input v-if="!klik.opslagknop" class="primary toekenknop" style="display: inline" type="button"
						value="Werklink toekennen" @click="LinkBijVoor(k)">
				</div>
			</div>
			<div v-if="Wlinks" class="vraagcontainer">
				<h3 class="h3inline">
					Jouw afgeschermde werk links
				</h3>
				<div v-for="(klik, k) in KlikbaarAr" :key="k" class="linklijn">
					<a class="werklinknav" @click="naarWerklink(klik.url, klik.pasw)">
						{{ klik.naam }}
					</a>
				</div>
				<form v-if="formAan" ref="wlform" :action="formLink" method="POST" target="_blank">
					<input v-model="formLink" type="hidden" name="delink">
					<input v-model="formPW" type="hidden" name="paswoord">
					<input v-model="formEm" type="hidden" name="email">
					<input type="hidden" value="TOOB" name="codepas">
					<input type="submit" value="test">
				</form>
			</div>
			<div v-if="Plinks" class="vraagcontainer">
				<h3 class="h3inline">
					Jouw favorieten: {{ gekozenPCategorie }}
				</h3>
				<input :disabled="false" type="button" text="Een link toevoegen" button-id="new-Aanvragen-button"
					class="icon-add toevoegknopje" @click="newPNote">
				<div v-for="(klik, k) in KlikbaarAr" :key="k" class="linklijn">
					<a :href="klik.url" target="_blank">{{ klik.naam }}</a>
					<input type="button" class="icon-clippy copknopje" title="kopieer link naar klembord"
						@click="klembord(klik.url, $event)">
					<input type="button" title="Delete" class="icon-delete delknopje"
						@click="linkWeg(klik.id, 'persoonlijk')">
				</div>
			</div>
		</NcAppContent>
		<NcAppSidebar v-show="show" title="Reserveerbaar:" @close="close">
			<input type="button" class="primary" style="margin-left:10px" value="Toon enkel wat nog vrij is"
				@click="nogbeschikbaar(gekozencat)">
			<NcAppNavigationItem v-for="mat in MateriaalAr" :key="mat.id" :title="mat.naam"
				@click="beschikbaar(mat.categorie, mat.id, mat.naam)">
				:name="mat.naam"
				<template slot="actions">
					<ActionButton icon="icon-details" @click="meerinfo(mat.omschrijving)">
						{{ mat.omschrijving }}
					</ActionButton>
				</template>
			</NcAppNavigationItem>
